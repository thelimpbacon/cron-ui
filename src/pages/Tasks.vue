<template>
  <div>
    ...
    <div class="">
      <div
        class="max-w-7xl mx-auto pt-12 px-4 sm:px-6 lg:pt-16 lg:px-8 lg:flex lg:items-center lg:justify-between border-b-4 border-gray-600"
      >
        <div class="flex">
          <h2
            class="pb-4 text-2xl font-extrabold tracking-tight text-gray-300 sm:text-3xl"
          >
            <span class="block">Stats</span>
          </h2>
          <div class="relative h-12 w-72">
            <img
              class="w-96 absolute -bottom-1"
              v-if="!loading"
              src="../assets/cat_sit.gif"
              alt=""
            />
            <img
              class="w-96 absolute -bottom-1"
              v-if="loading"
              src="../assets/cat_load.gif"
              alt=""
            />
          </div>
        </div>
      ...
      <div
        class="flex max-w-7xl mx-auto pt-4 lg:pt-8 lg:flex lg:items-center lg:justify-between"
      >
        <Stat
          v-for="stat in stats"
          :key="stat.title"
    ...
    <div class="">
      ...
      >
        <div class="flex">
          <h2
            class="text-2xl font-extrabold tracking-tight text-gray-300 sm:text-3xl"
          >
        ...
        <div class="flex">
          <a
            href="/create-task"
            class="ml-auto mr-4 my-auto flex items-center justify-center px-4 py-2 nes-btn is-success"
          >
            Create Task
          </a>
          <div
    ...
    <div class="flex items-center justify-center py-8 px-0 sm:px-6 lg:px-8">
      <section
        class="max-w-7xl w-1/2 space-y-8 my-12"
      ...
      <section v-if="tasks.length > 0" class="max-w-7xl w-full">
        <div
          class="nes-container with-title is-rounded is-dark w-full mb-4 min-w-full"
        ...
        >
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex nes-badge">
              <span class="is-dark" style="font-size: 8pt">{{
                task.cadence
              }}</span>
            </div>
            <div class="flex">
            ...
            <div class="flex" v-if="isTaskOwner(task) || isAuthed">
              <button
                @click.prevent="checkDeleteTask(task)"
                class="nes-badge mr-4"
                v-if="isTaskOwner(task)"
              >
                <span class="flex is-error">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="ml-auto mr-2 h-5 w-5"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
        ...
        <div v-if="tasks.length < totalTasks && !loading">
          <button
            ...
            class="mx-auto flex items-center justify-center px-8 py-3 nes-btn is-success md:py-4 md:text-lg md:px-10"
          >
            Load More Tasks
          </button>
        </div>
    ...
    <section v-if="deleteTaskModal == true">
      <dialog
        ...
        role="dialog"
        style="z-index: 100149; top: 20vh; position: fixed"
        open=""
      >
        ...
        <div class="dialog-menu mt-4 flex justify-between">
          ...
          <button @click.prevent="confirmDeleteTask" class="nes-btn is-success">
            Confirm
          </button>
        </div>
      </dialog>
...
export default {
  ...
  computed: {
    isAuthed() {
      ...
      if (this.accountId) {
        if (this.network === "mainnet" && this.accountId.search("near") > -1)
          isSameNetwork = true
  ...
  methods: {
    ...
    async confirmDeleteTask() {
      ...
      const croncat = await this.$near.getContractInstance(
        contract_id,
        abis.abis.manager
      )
      if (!croncat) return
      ...
      try {
        await croncat.remove_task(
          { task_hash: this.deleteTaskHash },
          300000000000000,
          0
        )
        this.deleteTaskModal = false
